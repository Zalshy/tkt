...
// TestRoleDelete_InUse verifies that deleting a role with an active session fails.
func TestRoleDelete_InUse(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected delete to fail for in-use role, got nil")
	}
	if !strings.Contains(err.Error(), "in use") {
		t.Fatalf("expected 'in use' in error, got: %v", err)
	}
}
