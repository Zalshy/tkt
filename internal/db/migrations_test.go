...
// a no-op: schema_version stays 13 and pre-seeded data survives.
...
// Open must see version=13 and skip all migrations without error.
...
// TestMigration_V7_VerifyFunction calls verifyV8Backfill directly with a tx
...
func TestMigration_V7_VerifyFunction(t *testing.T) {
	...
	// has no usage rows (source=0). verifyV8Backfill checks source != backfilled and
	...
	verifyErr := verifyV8Backfill(tx)
	if verifyErr == nil {
		t.Fatal("expected error from verifyV8Backfill, got nil")
	}
...
// TestMigration_V9_VerifyFunction calls verifyV10Backfill directly with a tx
...
// it within a transaction (which is rolled back) so verifyV10Backfill can run.
func TestMigration_V9_VerifyFunction(t *testing.T) {
	...
	// Recreate ticket_log_new within the transaction so verifyV10Backfill can query it.
	...
	verifyErr := verifyV10Backfill(tx)
	if verifyErr == nil {
		t.Fatal("expected error from verifyV10Backfill, got nil")
	}
	if !strings.Contains(verifyErr.Error(), "count mismatch") {
		t.Errorf("expected error to contain 'count mismatch', got: %v", verifyErr)
	}
...
func TestMigration_V11_VerifyFunction(t *testing.T) {
	...
	// Recreate it within the transaction so verifyV12Backfill can query it.
	...
	verifyErr := verifyV12Backfill(tx)
	if verifyErr == nil {
		t.Fatal("expected error from verifyV12Backfill, got nil")
	}
	if !strings.Contains(verifyErr.Error(), "count mismatch") {
		t.Errorf("expected error to contain 'count mismatch', got: %v", verifyErr)
	}
...
func TestMigration_V12_Backfill(t *testing.T) {
	...
	// with the row, so V11's verifyV12Backfill passes and V12 can run cleanly.
	for _, stmt := range []string{
		`DROP INDEX IF EXISTS idx_ticket_dependencies_depends_on`,
		`DROP TABLE IF EXISTS ticket_dependencies`,
