...
func TestListActive_FiltersVerifiedAndCanceled(t *testing.T) {
	...
	todo, err := Create("todo ticket", "", "standard", actor, sqlDB, "", 0)
	if err != nil {
		t.Fatalf("Create todo: %v", err)
	}
	planning, err := Create("planning ticket", "", "standard", actor, sqlDB, "", 0)
	if err != nil {
		t.Fatalf("Create planning: %v", err)
	}
	inProgress, err := Create("in_progress ticket", "", "standard", actor, sqlDB, "", 0)
	if err != nil {
		t.Fatalf("Create in_progress: %v", err)
	}
	done, err := Create("done ticket", "", "standard", actor, sqlDB, "", 0)
	if err != nil {
		t.Fatalf("Create done: %v", err)
	}
	verified, err := Create("verified ticket", "", "standard", actor, sqlDB, "", 0)
	if err != nil {
		t.Fatalf("Create verified: %v", err)
	}
	canceled, err := Create("canceled ticket", "", "standard", actor, sqlDB, "", 0)
	if err != nil {
